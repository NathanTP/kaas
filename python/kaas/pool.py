...
class Pool():
    ...
    async def getStats(self):
        ...
        mergePerClientStats(stats, policyStats)
        mergePerClientStats(stats, self.stats)
    ...
    async def run(self, funcName, nReturn, clientID, inputRefs, args, kwargs={}):
        ...
        with profiling.timer("t_policy_run", self.stats[clientID]):
            with profiling.timer('t_policy_wait_input', self.stats[clientID]):
            ...
            runActor = None
            retries = 0
            with profiling.timer("t_policy_wait_runner", self.stats[clientID]):
            ...
            with profiling.timer("t_policy_runner_invoke", self.stats[clientID]):
...
class PolicyAffinity(Policy):
    ...
    async def getStats(self):
        ...
        for poolStat in poolStats:
            mergePerClientStats(stats, poolStat)
