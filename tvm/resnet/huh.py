...
def runModel(inp, mode='direct'):    
    ...
    #kernel 0
    output_size = 802816
    nodes.append(kaas.bufferSpec('19', output_size, const=True, ephemeral=False))
    ...
    #kernel 0
    output_size = 8
    nodes.append(kaas.bufferSpec('167', output_size, const=True, ephemeral=False))
    ...
    #kernel 0
    output_size = 4004
    nodes.append(kaas.bufferSpec('171', output_size, const=True, ephemeral=False))
    ...
    c = np.frombuffer(libffCtx.kv.get('167'), dtype=np.int64)
    print(c)
...
from infbench import model
from infbench import dataset
...
def getInput():
    loader = resnet50.imageNetLoader(pathlib.Path.cwd() / 'data')
