...
def main(graph, code):
    ...
    metadata = json.load(open("tvm_meta.json")) 
    ...
    for i in range(1, len(graph["nodes"])):
        ...
        if node["op"] == "null":
            ...
            for j in range(len(kernels[node["name"]]) - 1):
                ...
                text += tab() + "imm.append(kaas.bufferSpec('a" + str(imm_count) + "', output_size))\n"
                text += tab() + "inputs = ?\n"
                text += tab() + "shapes = ?\n"
                text += tab() + "kerns.append(makeKern('" + kernels[node["name"]][j] + "', nodes[" + str(j) + "], path, inputs, shapes))\n"     
                imm_count += 1
            ...
            text += tab() + "nodes.append(kaas.bufferSpec(" + str(i) + ", output_size))\n" 
            text += tab() + "inputs = ?\n"
            text += tab() + "shapes = ?\n"
            kernel_name = kernels[node["name"]][len(kernels[node["name"]]) - 1]        
            text += tab() + "kerns.append(makeKern('" + kernel_name + "', nodes[" + str(i) + "], path, inputs, shapes))\n"     
